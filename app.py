...
def generate():
    ...
    try:
        ...
        if not genai or not GEMINI_API_KEY:
            ...
            session["product_idea"] = text  # Store the original product idea for chat context
...
def persona_reply(persona_id):
    ...
    persona = next((p for p in personas if p.get("id") == persona_id), None)
    if not persona:
        print(f"⚠️ Persona {persona_id} not found in session. Available personas: {[p.get('id') for p in personas]}")
    ...
    if not genai or not GEMINI_API_KEY:
        error_msg = (
            "⚠️ Gemini API is not configured. Please set GEMINI_API_KEY in your .env file. "
            "Get your API key from: https://makersuite.google.com/app/apikey"
        )
        print(error_msg)
        return jsonify({"error": error_msg}), 503

    # Build context about the product idea
    product_context = f"\nThe product idea you were reviewing was: \"{product_idea}\"" if product_idea else ""
    
    chat_prompt = f"""
You are {tone_description}. You are having a conversation about a product idea you reviewed.

Earlier you provided this feedback about the product:
"{review_summary}"
{product_context}

Respond to the user's latest message in a conversational, authentic tone that reflects this persona.
Stay in character and respond as this persona would. Be opinionated and specific when appropriate.
Keep your response concise (2-4 sentences typically) and natural.

User: {user_msg}
"""

    try:
        print(f"💬 Generating chat reply for persona {persona_id}: {user_msg[:50]}...")
        model = genai.GenerativeModel("gemini-2.0-flash-exp")
        response = model.generate_content(chat_prompt)
        reply = (response.text or "").strip()
        if not reply:
            reply = "I'm reflecting on that — could you clarify a bit more?"
        print(f"✅ Chat reply generated successfully")
