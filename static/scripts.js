(() => {
  ...
  function buildPersonaCard(review) {
    ...
    // Make entire card clickable to open chat
    el.addEventListener("click", (event) => {
      // Don't navigate if clicking on buttons or links
      if (event.target.closest(".persona-actions") || event.target.closest("button") || event.target.closest("a")) {
        return;
      }
      window.location.href = `/chat/${personaId}`;
    });

    el.querySelector(".btn-secondary-outline")?.addEventListener("click", (e) => {
      e.stopPropagation();
      startCall(name);
    });
    
    // Prevent card click when clicking the chat link (it will navigate naturally)
    el.querySelector("a.feedback-card-btn")?.addEventListener("click", (e) => {
      e.stopPropagation();
    });
    ...
    chatForm.addEventListener("submit", async event => {
      ...
      try {
        ...
        if (res.ok && data.reply) {
          appendAiMessage(chatWindow, personaName, data.reply);
        } else {
          const errorMsg = data.error || "⚠️ There was a problem connecting to the persona.";
          console.error("Chat API error:", errorMsg, data);
          appendAiMessage(chatWindow, personaName, errorMsg);
        }
      } catch (err) {
